...
def make_span(
    ...
    span = {
        ...
        "name": name,
        "kind": kind,
        "attributes": [{"key": k, "value": make_value(v)} for k, v in attributes.items()],
    }
    if parent_span_id:
...
def build_otlp(state: RunState, trace_ctx: dict[str, str | None]) -> dict[str, Any]:
    ...
    spans.append(make_span(
        ...
        "POST /v2/incidents",
        SPAN_KIND_SERVER,
        trace_ctx.get("incoming_parent_span_id"),
        common,
    ))
    spans.append(make_span(
        agent_span_id,
    ...
    spans.append(make_span(
        ...
        agent_span_id,
        {
            **common,
            ...
            "gen_ai.request.model": trace_ctx.get("model_name", "starter-local-model"),
        },
    ))
    ...
    for dispatch in state.actionLog:
        ...
        receipt = next(
            (
                r for r in state.receiptLog
                if getattr(r, "actionId", None) == dispatch.actionId
                and getattr(r, "attempt", None) == dispatch.attempt
            ),
            None,
        )
